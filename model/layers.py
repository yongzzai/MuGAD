...
class PreprocessLayers(nn.Module):
    ...
    def __init__(self, edge_dim, hidden_dim, emb_dim=None):
        super().__init__()

        self.num_attr = edge_dim-1
        self.hidden_dim = hidden_dim
        self.emb_dim = emb_dim if emb_dim is not None else hidden_dim

        # Activity Embedding  (Node)
        self.Act_Embedder = nn.Embedding(200, self.emb_dim)     # >> Shape(-1, emb_dim)

        self.pe = gnn.PositionalEncoding(emb_dim)           # Positional Encoding   (Edge)
        
        # Position projection layer if emb_dim != hidden_dim
        if self.emb_dim != self.hidden_dim:
            self.pos_proj = nn.Linear(self.hidden_dim, self.emb_dim)
        else:
            self.pos_proj = None
        
        if self.num_attr>0:
            self.Attr_Embedder = nn.ModuleList(
                    [nn.Embedding(300, self.emb_dim) for _ in range(self.num_attr)])    # Attribute Embedding (Edge)
        # >> Shape(-1, num_attr*emb_dim)
        
        self.NodeTransform = nn.Sequential(
            nn.Linear(self.emb_dim, hidden_dim),
            nn.LeakyReLU(),
            nn.Dropout(p=0.1),
            nn.Linear(hidden_dim, hidden_dim)) # >> Shape(num_nodes, hidden_dim)
        
        if self.num_attr>0:
            self.EdgeTransform =  nn.Sequential(
                nn.Linear(int(self.num_attr * self.emb_dim), hidden_dim),
                nn.LeakyReLU(),
                nn.Dropout(p=0.1),
                nn.Linear(hidden_dim, hidden_dim))     # >> Shape(num_edges, hidden_dim)
...
class Encoder(nn.Module):
    def __init__(self, edge_dim, hidden_dim, num_conv_layers, emb_dim=None):
        super().__init__()

        self.PreLayers = PreprocessLayers(edge_dim, hidden_dim, emb_dim)
