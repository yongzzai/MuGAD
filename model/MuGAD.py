...
class MuGAD():
    ...
    def Mutli_Task_Pretrain(self, edge_dim, graphs, label_idx):
        ...
        encoder = Encoder(edge_dim, self.hidden_dim, self.num_layers, self.emb_dim).to(self.device)
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=self.pretrain_epoch, eta_min=self.lr*0.1)
    ...
    def Adaptation(self, encoder, decoder, classifier, total_graphs):
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer, T_max=self.adapt_epoch, eta_min=self.lr*0.1)
