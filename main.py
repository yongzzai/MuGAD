...
parser.add_argument('--emb_dim', type=int, default=None, help='Embedding dimension for nn.Embedding layers')
...
if __name__ == '__main__':
    ...
    logging.info("")
    logging.info(f"hidden_dim: {args.hidden_dim}, emb_dim: {args.emb_dim}, num_conv_layer: {args.num_conv_layer}")
    ...
    # if model_args['batch_size'] < 64:
    #     dataset_names = [n for n in dataset_names 
    #                      if 'Billing' not in n
    #                      and 'Road' not in n]

    print(f'{model_args["batch_size"]} - {dataset_names}')
    ...
    for name in dataset_names:
        ...
        logging.info(f"Using batch size: {model_args['batch_size']}")
