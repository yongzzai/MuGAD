...
from generator.generation.anomaly import AttributeAnomaly
from generator.generation.attribute_generator import CategoricalAttributeGenerator
from generator.generation.example_values import *

from generator.generation.event_log_generator import EventLogGenerator
from generator.generation.model import ProcessMap
...
def generate_for_process_model(process_model, size=5000, anomalies=None, anomaly_p=0.3, num_attr=0,
    ...
    for i, (seed, params) in tqdm(enumerate(zip(seeds, parameters), start=1), desc=process_model, total=len(seeds)):
        ...
        if dir is None:
            ...
            event_log.save(process_model, postfix, anom_p, i, dir)